...
const BottomPlayer = ({ tracks, trackIndex, setTrackIndex, fullView, setFullView, isMobile }) => {
  ...
  useEffect(() => {
    volumeAvailable &&
      volumeUpKeypress &&
      changeVolume(volumeProgress + 0.1 > 1 ? 1 : volumeProgress + 0.1);
    volumeAvailable &&
      volumeDownKeypress &&
      changeVolume(volumeProgress - 0.1 < 0 ? 0 : volumeProgress - 0.1);
    volumeAvailable &&
      volumeKeypress &&
      changeVolume(volumeProgress === 0 ? 1 : 0);
  ...
  return (
    <div className="bottom-music-player">
      ...
      <div
        className="timers"
        style={tracks.length === 0 ? { color: "#818181" } : { color: "#fff" }}
      >
        {trackProgress ? formatTime(trackProgress) : "0:00"} /{" "}
      ...
      <div className="volume-container">
        <button
          ...
          aria-label="Volume"
          onClick={() =>
            volumeProgress === 0 ? changeVolume(1) : changeVolume(0)
          }
